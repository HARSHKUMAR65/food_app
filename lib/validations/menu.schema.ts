...
export const createMenuItemSchema = z.object({
  name: z.string().trim().min(2, "Menu item name must be at least 2 characters"),
  description: z
    .string()
    .trim()
    .min(5, "Menu item description must be at least 5 characters"),
  price: z.number().int().positive("Menu item price must be greater than 0"),
  image: z.string().trim().min(1, "Menu item image is required"),
